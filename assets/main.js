...
function createReminder(event) {
    ...
    if (event && event.target && event.target.id === 'toolbar-new-reminder') {
        ...
        newListGroup.innerHTML = `
            ...
            <button class="button-margin-bottom" onclick="createReminder(event)">New Reminder</button>
            <br/>
        `;
    ...
    // Check if the "New Reminder" button within a list group triggered the function
    if (event && event.target && event.target.closest('.list-group')) {
